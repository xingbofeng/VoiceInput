...
struct ASRFileTranscriptionWorker: FileTranscriptionWorking {
    ...
    func transcribe(
        ...
        let engine = asrManager.makeEngine(type: asrManager.effectiveSelectedEngineType)
        ...
        do {
            ...
            try feed(audioFile: audioFile, to: engine, progress: progress)
            engine.endAudio()
...
final class FileTranscriptionViewModel: ObservableObject {
    ...
    init(
        ...
        worker: any FileTranscriptionWorking = ASRFileTranscriptionWorker(),
