...
final class SQLiteHistoryRepository: HistoryRepository {
        ...
        try databaseQueue.write { connection in
            let statement = try connection.prepare(
                ...
                INSERT INTO dictation_history (
                    ...
                    target_app_name, processing_warnings_json, created_at, updated_at,
                    deleted_at
                )
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ...
    private func bind(_ entry: DictationHistoryEntry, to statement: SQLiteStatement) throws {
        ...
        try statement.bind(formatter.string(from: entry.createdAt), at: 14)
        try statement.bind(formatter.string(from: entry.updatedAt), at: 15)
        try statement.bind(entry.deletedAt.map(formatter.string(from:)), at: 16)
    }
    ...
    private func row(from statement: SQLiteStatement) throws -> DictationHistoryEntry {
        ...
        let deletedAt = statement.columnString(at: 15).flatMap(formatter.date(from:))
